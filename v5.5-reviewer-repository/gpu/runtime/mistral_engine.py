...
DEFAULT_LAYERS = (4, 8, 12, 20, 28, 36, 40)
PADDING, WIDTH, RANK = 1024, 5120, 16
...
def layers_contract(capture_layers, replacements):
    ...
    for layer in (*layers, *replacements):
        require(type(layer) is int and 4 <= layer <= 40,
                "Only decoder-output event hooks at layers 4 through 40 are allowed")
...
class Engine:
    def __init__(self, model_path=None, deadline_seconds=14400, *, model_receipt=None,
        ...
        require(versions == EXPECTED_PACKAGES, "Pinned v5 numerical/tokenization package versions required")
        import numpy as np
        import torch
        ...
        require(gpu.total_memory >= 75 * 1024**3, "At least 75 GiB physical GPU memory required")
        ...
        require((receipt_hash is not None or self.model.config._commit_hash == REVISION) and
                self.model.config.hidden_size == WIDTH and len(self.layers) == 40,
        ...
        self.environment = dict(model=MODEL, revision=REVISION, model_source=source,
                                ...
                                hardware_matches_v5_not_claimed=True,
    ...
    def load_basis(self, path, expected_sha):
        ...
        with self.np.load(path, allow_pickle=False) as archive:
            require(archive.files == ["rank_16"], "Unexpected saved basis array keys")
