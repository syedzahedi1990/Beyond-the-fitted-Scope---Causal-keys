...
WIDTH, PADDING, N_LAYERS, FIRST_LAYER = 5120, 1024, 40, 5
...
def validate_runtime(engine):
    ...
    paths = {"modeling_mistral.py": modeling, "sdpa_attention.py": inspect.getsourcefile(sdpa_module)}
    ...
    for index, block in enumerate(engine.layers):
        ...
        require(not block.training and not attention.training and block.hidden_size == WIDTH and
                ...
                attention.attention_dropout == 0 and getattr(attention.config, "sliding_window", None) is None,
        ...
        for name, shape in (("q_proj", (Q_HEADS * HEAD_DIM, WIDTH)),
            ...
            require(tuple(module.weight.shape) == shape and module.bias is None and
...
PARENT_ADAPTER_SHA256 = "a913f82a1663baff2a2a426753c44a503acf22b3036d839dd72815fe9a8b6f20"
...
class RelayEdgeEngine:
    ...
    def __init__(self, engine):
        ...
        self.adapter_provenance={'parent_adapter_sha256':PARENT_ADAPTER_SHA256,'source_sha256':file_sha(__file__),
                                 'key_site':'k_proj output before native reshape/RoPE','value_site':'v_proj output',
